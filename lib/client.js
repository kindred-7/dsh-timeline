...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    if (typeof document !== "undefined") {
      const tagId = "dsh-conversation-timeline/styles";
      if (document.querySelector(`style[data-plugin-css="${tagId}"]`) === null) {
        const tag = document.createElement("style");
        tag.dataset.plugin = "dsh-conversation-timeline";   // 标记来源插件
    ...
    function ConversationTimeline({ useSession }) {
      ...
      react.useEffect(() => {
        const handleScroll = () => {
          // 找到会话滚动容器（由宿主渲染时打上的 data-conversation-scroll 标记）
          const chatContainer = document.querySelector('[data-conversation-scroll]');
          if (!chatContainer || !userMessages.length) return;
          ...
          for (let i = userMessages.length - 1; i >= 0; i--) {
            ...
            // 用 data-chat-anchor-key 定位该消息在页面中的真实 DOM 行
            const row = document.querySelector(`[data-chat-anchor-key="${CSS.escape(msg.key)}"]`);
            if (row) {
      ...
      // --- 3.9 渲染守卫：无会话数据或没有用户消息时不渲染任何内容 ---
      if (!chat || userMessages.length === 0) {
        return null;
      }

      // --- 3.10 渲染：Fragment 包裹两块内容 ---
    ...
    function apply(ctx) {
      ...
      // 4.2 动态定位：时间轴是 fixed 定位，left 必须以对话容器为基准。
      //     以 .wSkVaW_viewArea（对话容器）为首选定位锚点：
      //     侧边栏拉宽/收窄时容器左缘随之移动，时间轴实时跟随，
      //     不再出现被拉宽后的侧边栏覆盖的问题。
      if (typeof document !== "undefined") {
        let observedEl = null;      // 当前被观察的容器元素
        let resizeObserver = null;  // 惰性创建的尺寸观察器

        // ------------------------------------------------------------------
        // 对话容器探测：不依赖具体类名。
        // wSkVaW_ 这类前缀是构建产物 hash，每台机器/每个版本都可能不同，
        // 因此按「稳定 data 属性 → 结构语义 → 已知类名」三级降级探测。
        // ------------------------------------------------------------------
        const findConversationEl = () => {
          // 第一级（跨版本通用）：宿主渲染每条消息时都带稳定的
          // data-chat-anchor-key 属性。从任一消息行向上找第一个
          // 纵向可滚动（overflow-y: auto/scroll）的祖先，即为会话容器。
        ...
        const adjustPosition = () => {
          ...
          if (!container) return; // 时间轴尚未挂载则跳过

          const conversationEl = findConversationEl();

          if (conversationEl) {
            // 时间轴贴对话容器左缘再留 20px 间距；
            // 容器左缘随侧栏宽度移动，时间轴同步移动，避免重叠
            const rect = conversationEl.getBoundingClientRect();
            container.style.left = (rect.left + 20) + "px";

            // 用 ResizeObserver 观察容器几何变化（侧栏拖宽、窗口缩放、折叠），
            // 比 setTimeout 轮询更及时；元素更换时先解绑旧观察
            if (!resizeObserver) {
              resizeObserver = new ResizeObserver(adjustPosition);
            }
            if (observedEl !== conversationEl) {
              if (observedEl) resizeObserver.unobserve(observedEl);
              observedEl = conversationEl;
              resizeObserver.observe(conversationEl);
            }
          }
        };

        // 延时两次兜底：等宿主异步渲染完成后再校正位置
