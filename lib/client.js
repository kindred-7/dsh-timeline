...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function ConversationTimeline({ useSession }) {
      ...
      //     不用 row.scrollIntoView({ block: 'start' }) 的原因：
      //       a) 它会沿祖先链逐级滚动所有可滚动容器，可能连带外层布局；
      //       b) 目标 scrollTop 超出 maxScrollTop 时被浏览器静默钳制——
      //          对话滚到最底部时，最后几条消息的"顶部对齐"位置超出可滚
      //          上限，点击纹丝不动，看起来就像坏了（本 bug 的根因）；
      //       c) 无法感知"零位移"情形，也就无从给出任何反馈。
      //     因此改为：显式找到宿主滚动容器，手动计算并钳制目标位置后
      //     scrollTo；钳制后与当前位置重合（物理上已不可滚）则闪烁高亮
      //     目标行作为"已在此处/已到底"的定位反馈。
      const handleClick = react.useCallback((key) => {
        ...
        // 宿主滚动容器：优先用显式标记，退回通用探测（与 3.9 定位共用逻辑）
        const container =
          document.querySelector("[data-conversation-scroll]") || findConversationEl();
        ...
        // 目标：消息行顶缘对齐容器顶缘（留 12px 呼吸边距），并钳制在合法范围
        const cRect = container.getBoundingClientRect();
        const rRect = row.getBoundingClientRect();
        const maxTop = Math.max(container.scrollHeight - container.clientHeight, 0);
        const target = Math.min(
          Math.max(container.scrollTop + (rRect.top - cRect.top) - 12, 0),
          maxTop,
        );

        if (Math.abs(target - container.scrollTop) < 1) {
          // 零位移：目标行已在可达极限（如已在底部点最后一条），闪烁提示代替无效滚动
          flashRowTarget(row);
          return;
        }
