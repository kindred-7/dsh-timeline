...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = `
      ...
      /* ---- 悬浮提示框：显示该条消息的提问全文 ----
              视觉对齐 DSH 原生浮层（ui-theme design tokens）：
              深色板 rgb(44,44,46) 深浅主题通用、纯白文字、13px/20px、大圆角。
              最多显示 3 行，超出以省略号截断（-webkit-line-clamp）。 ---- */
      .timeline-tooltip {
        position: fixed;             /* 跟随鼠标坐标绝对定位 */
        box-sizing: border-box;      /* 显式声明：宿主全局可能已设 border-box，
                                        max-height 必须把内边距算进去才能整行收口 */
        background-color: var(--dsw-alias-tooltip-bg, rgb(44, 44, 46));
        color: var(--dsw-static-neutral-bluish-00, rgb(255, 255, 255));
        padding: 10px 12px;
        border-radius: 10px;
        font-size: 13px;             /* 与原生 Tooltip 一致 */
        line-height: 20px;           /* 与原生 Tooltip 一致 */
        max-width: 280px;            /* 长提问自动折行 */
        max-height: 80px;            /* 3 行 × 20px + 上下内边距 20px = 80px，
                                        恰好整行收口，不会切出半行 */
        overflow: hidden;
        display: -webkit-box;
        -webkit-line-clamp: 3;
        -webkit-box-orient: vertical;
        overflow-wrap: break-word;   /* URL/路径等不可断 token 不得撑破宽度 */
    ...
    // --------------------------------------------------------------------------
    // 2.3 工具函数：tooltip 文本 JS 侧截断（CSS line-clamp 之外的兜底）。
    //     按"显示宽度单位"预算：全角/CJK 记 2、半角记 1，110 单位 ≈ 3 行
    //     （280px 板宽下每行约 37 个半角或 19 个全角字符）。超预算在最近的
    //     字符处收尾并追加省略号——即使宿主环境令 CSS 截断失效，也不会溢出。
    // --------------------------------------------------------------------------
    const TOOLTIP_MAX_UNITS = 110;
    function clampTooltipText(text) {
      if (!text) return text;
      let units = 0;
      for (let i = 0; i < text.length; i++) {
        const code = text.codePointAt(i);
        if (code > 0xffff) i++; // 代理对（emoji 等）整体计一次
        units += code > 0x2e7f ? 2 : 1; // CJK/全角区记 2，其余记 1
        if (units > TOOLTIP_MAX_UNITS) {
    ...
    // --------------------------------------------------------------------------
    // 2.5 历史回填：浏览器本地历史存储（localStorage）。
    //     记录本浏览器提交过的用户提问（含 steering 消息），按文本去重、
    //     新→旧排序、封顶 100 条。读取是同步的，↑ 键回填永不等待网络。
    // --------------------------------------------------------------------------
    const HISTORY_STORAGE_KEY = "@kindred7/dsh-timeline/history:v1";
    ...
    function historyLoad() {
      try {
        ...
        for (const item of parsed) {
          ...
          const ts = item.ts;
          if (typeof text !== "string" || text === "" || typeof ts !== "number") continue;
    ...
    function historySave(entries) {
      try {
        ...
        // 存储不可用/写满：历史捕获尽力而为，静默降级。
      }
    }

    // 记录一条提问：按 trim 后文本去重（命中即移到最前），超限裁掉最旧的。
    function historyRecord(text) {
      const trimmed = text.trim();
      if (trimmed === "") return;
      const rest = historyLoad().filter((entry) => entry.text.trim() !== trimmed);
      historySave([{ text: trimmed, ts: Date.now() }, ...rest].slice(0, HISTORY_MAX_ENTRIES));
    }
    ...
    function ConversationTimeline({ useSession }) {
      ...
      const handleMouseEnter = react.useCallback((e, message, index) => {
        ...
        setTooltip({
          ...
          y: rect.top,
          text: clampTooltipText(message.text) // JS 侧截断兜底（见 2.3）
        });
      }, []);
    ...
    // ==========================================================================
    // 4. 历史回填组件 HistoryRecall（终端式 ↑ 键召回，不弹面板）
    //
    //    交互约定：
    //      ↑（输入框为空）   直接把最近一条历史问题填进输入框
    //      ↑（连续按）       继续往前翻更早的历史
    //      ↓                 往回翻；翻过最新一条后恢复空白输入框
    //      任意手动编辑      结束浏览态，↑ 恢复「仅空框触发」
    //
    //    props.useInput     —— 宿主注入的输入机状态 hook（draft/draftRev/phase）
    //    props.inputActions —— 宿主注入的草稿写入动作（setDraft 等）
    // ==========================================================================
    function HistoryRecall({ useInput, inputActions }) {
      // 用 ref 镜像最新输入状态：keydown 监听只绑一次，回调里始终读到新值
      const input = useInput((s) => s);
      const stateRef = react.useRef(input);
      stateRef.current = input;

      // 浏览态游标：index -1 = 未在浏览；expected 记录我们上次填充的文本，
      // 与当前 draft 不一致即说明用户手动编辑过 → 结束浏览态。
      const browseRef = react.useRef({ index: -1, entries: [], expected: "" });
      ...
      react.useEffect(() => {
        const onKeyDown = (event) => {
          ...
          // 无修饰键 + 非输入法组合态才生效；已被他处消费的按键不抢
          if (event.defaultPrevented || event.isComposing || event.keyCode === 229) return;
          if (event.altKey || event.ctrlKey || event.metaKey || event.shiftKey) return;
          if (stateRef.current.phase !== "plain") return;

          const browse = browseRef.current;
          // 上次填充后用户手动编辑过 → 结束浏览态
          if (browse.index >= 0 && stateRef.current.draft !== browse.expected) browse.index = -1;

          const fill = (text) => {
            browse.expected = text;
          ...
          if (isUp) {
            if (browse.index < 0) {
              ...
              const entries = historyAll().map((entry) => entry.text);
              if (entries.length === 0) return;
              browse.entries = entries;
              browse.index = 0;
              return fill(entries[0]);
            }
            // 浏览态继续 ↑：往更早的历史走，到头即停
            if (browse.index < browse.entries.length - 1) {
              browse.index += 1;
              return fill(browse.entries[browse.index]);
            }
            return;
          }

          // ↓ 仅在浏览态有意义：往回走，越过最新一条恢复空白并退出浏览态
        ...
        // 捕获阶段监听：先于宿主 textarea 默认行为（光标移动）执行
        document.addEventListener("keydown", onKeyDown, true);
        return () => document.removeEventListener("keydown", onKeyDown, true);
      }, [inputActions]);

      return null; // 纯行为组件，无视觉输出
    }
    ...
    // 声明本插件依赖宿主的 "slots" 服务（插槽系统）与 "sessions" 服务（会话投影，
    // 用于捕获历史提问）；加载器据此注入 ctx.slots / ctx.sessions
    ...
    function apply(ctx) {
      ...
      // -----------------------------------------------------------------------
      // 5.1 历史提问捕获：订阅会话列表，逐会话监听消息快照，
      //     把定稿的 user / steering 消息文本写入 localStorage（每条只记一次）。
      // -----------------------------------------------------------------------
      const sessions = ctx.sessions;
      const seenKeys = new Set();
      const unsubs = new Map();

      // 提取一条消息的纯文本：多文本块以换行拼接后 trim（与 tooltip 的
      // messageText 不同，这里保留块间分隔，避免多段提问粘连）
      function harvestText(content) {
        if (!Array.isArray(content)) return "";
        return content
          .map((block) => (block && block.type === "text" ? block.text : ""))
          .join("\n")
      ...
      function harvest(sessionId, sessionFace) {
        const nodes = sessionFace.getSnapshot().nodes;
        if (!Array.isArray(nodes)) return;
        for (const node of nodes) {
          if (!node || (node.kind !== "user" && node.kind !== "steering")) continue;
          const key = String(sessionId) + ":" + node.kind + ":" + String(node.seq);
          if (seenKeys.has(key)) continue;
          seenKeys.add(key);
          const text = harvestText(node.content);
          if (text !== "") historyRecord(text);
        }
      }
      ...
      function watchSession(sessionId) {
        ...
        const harvestThis = () => harvest(sessionId, binding.session);
        harvestThis(); // 订阅前先收割一次存量消息
        unsubs.set(sessionId, binding.session.subscribe(harvestThis));
      }
      ...
      ctx.effect(() => {
        syncWatchers();
        const offList = sessions.list.subscribe(syncWatchers);
        return () => {
          offList();
          for (const off of unsubs.values()) off();
          unsubs.clear();
        };
      }, "dsh-timeline: history capture");
      // 连接重建后旧绑定全部失效；下一次 list 快照会重新挂监听
      ...
      // -----------------------------------------------------------------------
      // 5.2 历史回填注册：把 HistoryRecall 挂到输入框浮层槽位
      //     （与触发管线自己的 MenuView 并排；本组件渲染 null，纯行为）。
      // -----------------------------------------------------------------------
      ctx.slots.inject("conversation.input.overlay", () =>
        ctx.slots.register(
          {
            name: "conversation.input.overlay", // 输入框浮层槽位
            id: "history-up-recall",            // 本组件在该插槽内的 id
            order: 2                            // 排序权重（MenuView 为 0）
          },
          HistoryRecall
        )
      );
    ...
    // ---------------------------------------------------------------------------
    // 6. 导出：apply 为宿主必需入口；inject 声明依赖；组件另行导出便于测试/复用
    // ---------------------------------------------------------------------------
    exports.apply = apply;
    exports.inject = inject;
    exports.ConversationTimeline = ConversationTimeline;
    exports.HistoryRecall = HistoryRecall;
