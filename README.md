...
dsh web 对话增强插件：在对话界面左侧渲染一条极简的垂直刻度线，每一轮用户提问对应一个刻度，为长对话提供"全局地图"式的快速导航；同时内置终端式历史召回——输入框为空时按 ↑ 直接回填上一条提问，像终端一样翻阅历史。

## 功能描述

**一句话简介**（可直接用作 GitHub About / 仓库描述）：

> @kindred7/dsh-timeline —— dsh web 对话导航时间线：左侧刻度标记每轮提问，悬停预览、点击定位、滚动跟随；附终端式 ↑ 键历史召回。

**详细功能：**

- 🧭 **对话快速导航**：每条用户提问在对话区左侧对应一条短刻度线；点击任意刻度，平滑滚动定位到对应消息；滚动浏览时自动高亮当前阅读位置所在的刻度
- ⌨️ **终端式历史召回**：输入框为空时按 ↑ 直接回填最近一条提问；连续 ↑ 往前翻阅更早历史，↓ 往回翻，任意手动编辑即退出浏览态——不弹面板、不打断输入
...
输入框为空时按 ↑，即可像终端一样召回历史提问：

| 按键 | 行为 |
|---|---|
| ↑（输入框为空） | 直接把最近一条历史问题填进输入框（不弹面板） |
| ↑（连续按） | 继续往前翻更早的历史 |
| ↓ | 往回翻；翻过最新一条后恢复空白输入框 |
| 任意手动编辑 | 结束浏览态，↑ 恢复「仅空框触发」 |

- 历史记录保存在浏览器 localStorage：上限 100 条、按文本去重、新→旧排序
- 自动捕获本浏览器内所有会话提交过的用户提问（含 steering 消息）
- 无修饰键、非输入法组合态才生效，不干扰 `/` `@` 触发菜单
...
- **新增**：终端式历史召回——输入框为空按 ↑ 回填最近提问，↑/↓ 翻阅历史，手动编辑退出浏览态（详见「使用说明 → 历史召回」）
- **新增**：自动捕获会话提交的用户提问（含 steering 消息），localStorage 存储上限 100 条、文本去重、新→旧排序
